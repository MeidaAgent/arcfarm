...
export class ShopUI {
  ...
  rows() {
    ...
    if (this.tab === 'upgrade') {
      return this.shop.upgrades.map((u) => {
        ...
        return { upgrade: u, tools, label: `Upgrade tools → ${u.name}`, price: u.price, sub: u.effect };
      });
    }
    if (this.tab === 'buy') {
      return (this.shop.stock ?? [])
        ...
        .map((e) => {
          ...
          return { id: e.id, label: d?.name ?? e.id, price: d?.buy ?? 0, sub: d?.sell ? `sell G${d.sell}` : '' };
        });
    }
  ...
  switchTab() {
    ...
    // tidak menampung hasil panen; that is Sarah's store and the shipping bin's job.
  ...
  buy(row) {
    ...
    if (def?.tool && this.inv.count(row.id) > 0) { this.message = `You already own ${def.name}.`; this.audio?.denied(); return; }
    if (this.player.money < row.price) { this.message = 'Not enough money.'; this.audio?.denied(); return; }
    const extra = itemDef(row.id)?.capacity ? { water: 0 } : null;
    if (this.inv.add(row.id, 1, extra) > 0) { this.message = 'Bag is full.'; this.audio?.denied(); return; }
    ...
    this.message = `Bought ${def.name} — G${row.price}`
      + (def.eat ? '  ·  select in hotbar, press E to eat' : '');
  }
  ...
  sell(row) {
    ...
    this.message = `Sold ${itemDef(s.id).name} — G${total}`;
  ...
  upgrade(row) {
    ...
    if (!target) { this.message = 'No tools in bag.'; return; }
    const cur = target.s.level ?? 0;
    if (cur >= u.level) { this.message = `${itemDef(target.s.id).name} is already level ${cur}.`; return; }
    if (cur !== u.level - 1) { this.message = `Upgrade to level ${cur + 1} first.`; return; }
    if (this.player.money < u.price) { this.message = 'Not enough money.'; return; }
    for (const m of u.materials) {
      if (this.inv.count(m.id) < m.count) {
        this.message = `Need ${m.count}× ${itemDef(m.id)?.name ?? m.id}.`;
        return;
      }
    }
    for (const m of u.materials) {
    ...
    this.message = `${itemDef(target.s.id).name} is now ${u.name} level.`;
  }
  ...
  cycleTool() {
    ...
    this.message = `Tool selected: ${itemDef(tools[this.toolIndex].s.id).name}`;
  }
  ...
  draw(ctx) {
    ...
    text(ctx, this.player.money.toLocaleString('en-US'), x + W - 24, y + 42, { font: UI.fontBig, align: 'right' });
    ...
    if (this.shop.upgrades) {
      ...
      text(ctx, cur ? `Tool: ${itemDef(cur.s.id).name} (lv ${cur.s.level ?? 0})` : 'No tools',
        x + 24, y + 44, { font: UI.fontBig });
    } else if (this.shop.canSell === false) {
      const [o, c] = this.shop.open;
      text(ctx, `Open ${String(o).padStart(2, '0')}:00–${String(c).padStart(2, '0')}:00`,
      ...
      for (const [i, t] of ['buy', 'sell'].entries()) {
        ...
        text(ctx, t === 'buy' ? 'BUY' : 'SELL', tx + 44, y + 43, { font: UI.font, align: 'center' });
      }
    }
    ...
    const listY = y + 76;
    if (!rows.length) {
      text(ctx, this.tab === 'sell' ? 'Nothing to sell.' : 'No seasonal stock yet.',
    ...
    for (let i = 0; i < Math.min(ROWS_VISIBLE, rows.length); i++) {
      ...
      text(ctx, `G ${r.price.toLocaleString('en-US')}`, x + W - 26, ry + 12,
    ...
    const hint = this.shop.upgrades
      ? 'Arrows select  ·  Tab switch tool  ·  E confirm  ·  Esc close'
      : this.shop.canSell === false
        ? 'Arrows select  ·  E confirm  ·  Esc close'
        : 'Arrows select  ·  ←/→ buy/sell  ·  E confirm  ·  Esc close';
