...
export class InventoryUI {
  ...
  drop() {
    ...
    if (!this.canDrop(s)) { this.say(`${d?.name ?? s.id} cannot be discarded.`); return; }
    if (this.confirm !== i) {
      this.confirm = i;
      this.confirmT = 4;
      this.say(`Discard ${s.count}× ${d?.name ?? s.id}? Press Backspace again.`);
      return;
    }
    this.inv.removeAt(i, s.count);
    this.clearConfirm();
    this.say(`${d?.name ?? s.id} discarded.`);
  }
  ...
  draw(ctx) {
    ...
    panel(ctx, x, y, w, h);
    text(ctx, 'INVENTORY', x + w / 2, y + 20, { font: UI.fontTitle, align: 'center', color: UI.red });
    ...
    const info = sel ? `${itemDef(sel.id)?.name ?? sel.id}${itemDef(sel.id)?.sell ? ` — sell G${itemDef(sel.id).sell}` : ''}` : 'Empty';
    text(ctx, this.note || info, x + w / 2, y + h - 78,
      { font: UI.fontBig, align: 'center', color: this.note ? '#8a2c1d' : UI.outline });
    text(ctx, 'Arrows select  ·  E move item', x + w / 2, y + h - 52,
      { font: UI.font, align: 'center', color: '#5a4a30' });
    text(ctx, 'Backspace discard  ·  I / Esc close', x + w / 2, y + h - 32,
      { font: UI.font, align: 'center', color: '#5a4a30' });
  }
}
