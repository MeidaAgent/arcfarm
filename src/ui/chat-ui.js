...
export class ChatUI {
  ...
  ensureInput() {
    ...
    el.autocomplete = 'off';
    el.setAttribute('aria-label', 'Type a chat message');
  ...
  drawPanel(ctx) {
    ...
    const headY = p.y + 12;
    text(ctx, 'VILLAGE CHAT', p.x + 16, headY + 2, { font: UI.fontTitle, color: UI.red });
    if (this.chat.status === 'online') {
      ...
      text(ctx, this.chat.status === 'connecting' ? 'connecting...' : 'offline',
    ...
    if (!rows.length) {
      const msg = this.chat.status === 'offline'
        ...
        : 'Nobody is talking yet. Say hi first!';
    ...
    text(ctx, typed ? shown : 'Type a message...', p.x + 20 - over, iy + 6,
    ...
    text(ctx, 'Enter send  ·  Esc close  ·  wheel scroll history', p.x + p.w / 2, iy + inputH + 4,
  ...
  drawRow(ctx, r, left, right, y) {
    ...
    text(ctx, r.own ? 'You' : r.m.name, tx, y, { font: UI.font, align, color: r.own ? '#3f6f2a' : UI.red });
