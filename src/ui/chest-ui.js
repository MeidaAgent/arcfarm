...
export class ChestUI {
  ...
  draw(ctx) {
    ...
    text(ctx, 'STORAGE CHEST', x + w / 2, y + 18, { font: UI.fontTitle, align: 'center', color: UI.red });
    text(ctx, 'Your bag', x + 24, gy2 - 26, { font: UI.fontBig });
    ...
    text(ctx, sel ? itemDef(sel.id)?.name ?? sel.id : 'Empty', x + w / 2, y + h - 46, { font: UI.fontBig, align: 'center' });
    text(ctx, 'Arrows select  ·  E move  ·  Esc close', x + w / 2, y + h - 26, { font: UI.font, align: 'center', color: '#5a4a30' });
  }
}
