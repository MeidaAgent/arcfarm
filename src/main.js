...
const state = {
  ...
  profile: { name: 'Farmer', farmName: 'ARC FARM Farm' },
};
...
async function boot() {
  ...
  await loadAssets('assets/', (p) => { loadingEl.textContent = `Loading assets ${Math.round(p * 100)}%`; });
  ...
  loadingEl.remove();
  canvas.focus();
  if (fresh) showToast(`Welcome to ${state.profile.farmName}, ${state.profile.name}!`, 4);
...
function doSleep(passedOut) {
  ...
  const lines = [];
  if (earned) lines.push(`Sold G${earned.toLocaleString('en-US')}`);
  lines.push(`${state.time.dateText} · ${state.weather.name}`);
  showToast(passedOut ? 'You passed out from exhaustion... (50% energy)' : lines.join(' · '), 4.5);
}
...
function giveGift(npc) {
  if (npc.beast) { showToast(`${npc.name} just stares at you.`); state.audio.denied(); return; }
  const held = state.inv.held;
  const def = held && itemDef(held.id);
  if (!def) { showToast('Hold the item you want to give first.'); state.audio.denied(); return; }

  const r = state.social.give(npc.id, { ...def, id: held.id }, dayNumber());
  if (!r.ok) {
    showToast(r.why === 'tool' ? 'Tools cannot be given away.' : `${npc.name} already got a gift today.`);
    state.audio.denied();
    return;
  }
...
function interactTarget() {
  ...
  if (m.id === 'valley') {
    ...
    for (const [key, id, verb] of [
      ['storeDoor', 'shop', 'Shop at'],
      ['smithDoor', 'smith', 'Upgrade tools at'],
      ['warungDoor', 'warung', 'Order at'],
      ['townHallDoor', 'balai', 'Check the request board at'],
      ['clinicDoor', 'klinik', 'Get treated at'],
    ]) {
      const dpos = m[key];
      if (!dpos || f.c !== dpos.c || f.r !== dpos.r) continue;
      const place = state.shops[id];
      if (!place) return { kind: 'closed', label: 'Opening soon' };
      const hour = state.time.hour24;
      if (hour < place.open[0] || hour >= place.open[1]) {
        return { kind: 'shut', label: `${place.name} closed`, msg: place.closedMsg };
      }
      ...
      if (place.kind === 'requests') {
        ...
        const label = state.requests.done(day) ? `${place.name}`
          : ready ? `Deliver ${req.count} ${req.itemName}`
          : `${verb} ${place.name}`;
  ...
  for (const n of state.npcs) {
    ...
    return { kind: 'npc', npc: n, label: canGift ? `Talk · G to give gift` : `Talk to ${n.name}` };
  }
  for (const a of state.animals) {
    if (!inFront(a, 40)) continue;
    if (a.hasProduce) return { kind: 'collect', animal: a, label: `Collect ${itemDef(a.produce.item).name}` };
    if (a.produce && !a.fed) return { kind: 'feed', animal: a, label: 'Give feed' };
    return { kind: 'pet', animal: a, label: 'Pet' };
  }

  if (m.id === 'valley') {
    if (f.selfC === m.door.c && f.selfR === m.door.r && p.dir === 'up') return { kind: 'bed', label: 'Sleep until morning' };
    if (m.well && f.c >= m.well.c && f.c <= m.well.c + 1 && f.r === m.well.r) return { kind: 'well', label: 'Fill watering can' };
    if (m.bin && f.c >= m.bin.c && f.c <= m.bin.c + 1 && f.r === m.bin.r) return { kind: 'bin', label: 'Put in the shipping bin' };
    if (findObj(f.c, f.r, (o) => o.kind === 'notice')) return { kind: 'notice', label: 'Read the notice board' };
  }
  ...
  if (m.terrainAt(f.c, f.r) === 'water' && itemDef(state.inv.held?.id)?.tool === 'fish') {
    ...
    return { kind: 'fish', c: f.c, r: f.r, lucky: !!shadow, label: shadow ? 'Fish (biting!)' : 'Fish' };
  }

  const soil = state.farming.at(f.c, f.r);
  if (soil && state.farming.ripe(soil)) return { kind: 'harvest', c: f.c, r: f.r, label: 'Harvest' };
  const bush = findObj(f.c, f.r, (o) => o.kind === 'bush' && o.col === 1);
  if (bush) return { kind: 'forage', obj: bush, label: 'Pick berries' };
  const pick = findObj(f.c, f.r, (o) => o.kind === 'pick');
  if (pick) return { kind: 'pick', obj: pick, c: f.c, r: f.r, label: `Take ${itemDef(pick.drop)?.name ?? pick.drop}` };
  return null;
}
...
function spend(n) {
  if (state.player.energy < n) { showToast('Out of energy — get some rest.'); state.audio.denied(); return false; }
...
function give(id, count = 1) {
  ...
  if (left) { showToast('Inventory full!'); state.audio.denied(); return; }
  const p = state.player;
...
function useHeld() {
  ...
  if (d.seed) {
    if (!m.tillable(f.c, f.r)) { showToast('Seeds can only be planted on garden plots.'); return true; }
    const res = farm.plant(f.c, f.r, d.seed, state.time.season);
    if (res === 'season') { showToast(`${d.name} does not grow in this season.`); return true; }
    if (res) {
      state.inv.removeAt(state.inv.index, 1);
    ...
    showToast('Hoe the soil first.');
    return true;
  }

  switch (d.tool) {
    case 'hoe': {
      if (!m.tillable(f.c, f.r)) { showToast('You can only hoe the garden plots.'); return true; }
      if (m.isSolid(f.c, f.r) || findObj(f.c, f.r, (o) => o.kind === 'weed')) { showToast('Something is in the way.'); return true; }
    ...
    case 'water': {
      if ((held.water ?? 0) <= 0) { showToast('Watering can is empty — fill it at the well.'); return true; }
...
function doInteract() {
  ...
  switch (t.kind) {
    ...
    case 'bed':
      dialog.open({ text: 'Sleep until tomorrow morning?|(Press E to sleep — progress is saved)' });
    ...
    case 'well': {
      const held = inv.held;
      if (held && itemDef(held.id)?.capacity) { held.water = itemDef(held.id).capacity; showToast('Watering can refilled.'); }
      else showToast('Hold the watering can first.');
      break;
    }
    case 'bin': {
      const held = inv.held;
      if (!held) { showToast('Nothing to sell.'); break; }
      const def = itemDef(held.id);
      if (!def?.sell) { showToast('This item cannot be sold.'); break; }
      state.audio.coin();
      state.shipping.push({ id: held.id, count: held.count });
      showToast(`${held.count}x ${def.name} → G${def.sell * held.count} (paid tomorrow)`, 3);
    ...
    case 'notice': {
      ...
      dialog.open({
        ...
        text: `Today is ${state.time.dateText}, weather: ${state.weather.name}.|`
          + `Tomorrow's forecast: ${state.weather.nameOf(state.weather.next)}.`
          + (state.weather.next === 'rain' ? ' Rain will water your crops.' : ' Remember to water.')
          + '|' + (done || !req
            ? 'No errands waiting at the Village Hall today.'
            : `${req.npcName} left a request at the Village Hall: ${req.count} ${req.itemName}.`)
    ...
    case 'closed': showToast('Not open yet — coming soon.'); break;
    case 'harvest': {
    ...
    case 'feed':
      if (inv.count('feed') > 0) {
        ...
        t.animal.fed = true;
        showToast('Fed.');
      } else { t.animal.fed = true; showToast('Left to graze.'); }
      break;
...
function eatHeld(quiet = false) {
  ...
  if (!d?.eat) { if (!quiet) showToast('Not edible.'); return false; }
  const p = state.player;
  if (p.energy >= p.maxEnergy) {
    showToast(`Your energy is still full — save that ${d.name}.`);
    state.audio.denied();
    return true;
  }
  ...
  state.audio.harvest();
  showToast(`Ate ${d.name} (+${d.eat} energy)`);
  return true;
}
 ...
 * hilang, paying Dr. Rachel selalu lebih baik daripada tidur, dan malam
...
function visitClinic(place) {
  ...
  if (p.energy >= p.maxEnergy) {
    state.dialog.open({
      speaker: 'Dr. Rachel', portrait: 'portrait_npc_ratna',
      text: 'You look healthy. Keep your money, you will need it later.',
  ...
  if (p.money < place.price) {
    state.dialog.open({
      speaker: 'Dr. Rachel', portrait: 'portrait_npc_ratna',
      text: `Resting here costs G${place.price}. Not enough money yet — go home, sleep heals too.`,
      hearts: { filled: state.social.hearts('ratna'), max: MAX_HEARTS },
    });
    state.audio.denied();
    return;
  ...
  state.dialog.open({
    speaker: 'Dr. Rachel', portrait: 'portrait_npc_ratna',
    text: `Lie down for ${hilang} hours, G${place.price}?|(Press E to rest)`,
    hearts: { filled: state.social.hearts('ratna'), max: MAX_HEARTS },
  });
  state.dialog.onClose = () => {
    ...
    showToast(`Rested ${hilang} hours — energy restored.`, 3);
  };
}
...
function readRequestBoard() {
  const day = dayNumber();
  const req = state.requests.today(day);
  if (!req) { showToast('The request board is still empty.'); return; }
  if (state.requests.done(day)) {
    state.dialog.open({
      speaker: 'Village Hall',
      text: `You already fulfilled ${req.npcName}'s request today.|You have helped the villagers ${state.requests.total} times.`,
    });
    return;
  }
  const ready = state.requests.canFill(state.inv, day);
  state.dialog.open({ speaker: 'Village Hall', text: requestText(req, ready) });
  if (!ready) return;
  state.dialog.onClose = () => {
    ...
    showToast(`${filled.npcName} thanks you — G${filled.gold} and hearts gained.`, 3);
  };
}
...
function updateFishing(dt) {
  ...
  if (input.pressed('menu')) { f.cancel(); showToast('Line reeled in.'); return true; }

  if (f.state === 'done') {
    const r = f.take();
    if (r === 'missed') { showToast('It slipped away...'); state.audio.denied(); }
    else if (r === 'lost') { showToast('The fish got away!'); state.audio.denied(); }
    else {
...
function update(dt) {
  ...
  if (state.fadeDir === 0) {
    ...
    if (input.pressed('gift')) {
      ...
      else showToast('Face a villager first.');
    }
    ...
    if (input.pressed('zoom')) { const z = renderer.cycleZoom(); showToast(`Zoom: ${z ? '×' + z : 'auto ×' + view.zoom}`); }
  }
...
boot().catch((e) => { loadingEl.textContent = `Failed to load: ${e.message}`; console.error(e); });
