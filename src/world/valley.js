// ARC FARM — one single outdoor map holding the farm, the village, the
...
export function buildValley() {
  ...
  landmarks.push({ name: 'Shipping Bin', at: map.bin });

  obj('props', house.c - 1, house.r + 2, { kind: 'decor', col: 0 });     // mailbox

  // village side
  map.townHallDoor = building('town_hall', 'town_hall', 'Village Hall', plazaCx - 3, plaza.r0 - 3, 7, 3);
  for (let r = map.townHallDoor.r + 1; r < plaza.r0; r++) carve(plazaCx, r);

  map.storeDoor = building('store', 'store', 'Store', plaza.c0 - 8, mainRow - 3, 6, 3);
  for (let r = map.storeDoor.r + 1; r <= mainRow; r++) carve(map.storeDoor.c, r);

  map.warungDoor = building('warung', 'warung', 'Stall', plaza.c1 + 3, mainRow - 3, 5, 2);
  ...
  map.smithDoor = building('blacksmith', 'blacksmith', 'Blacksmith', plazaCx - 2, plaza.r1 + 3, 5, 2);
  ...
  map.clinicDoor = building('clinic', 'clinic', 'Clinic', spineC + 1, clinicRow, 6, 3);
  ...
  [spineC + 1, spineC + 7, spineC + 13].forEach((c, i) => {
    ...
    landmarks.push({ name: `House ${i + 1}`, at: { c: c + 2, r: homeRow } });
    doorTiles.push([c + 2, homeRow]);
  });

  // forest and shore
  map.raniDoor = building('hut_rani', 'hut', 'Rosie Hut', 14, 26, 4, 2);
  map.wulanDoor = building('hut_wulan', 'hut', 'Willow Hut', 33, beachRow + 1, 4, 2);
  ...
  landmarks.push({ name: 'Fountain', at: { c: plazaCx, r: plazaCy } });
  ...
  landmarks.push({ name: 'Notice Board', at: { c: plaza.c0 - 2, r: mainRow - 1 } });
  ...
  landmarks.push({ name: 'Forest Pond', at: { c: Math.round(pondCx), r: pond.r1 } });
  ...
  landmarks.push({ name: 'Pier', at: { c: dock.c, r: dock.r0 } });
  ...
  for (const [c, r, name] of [[forestLaneC + 1, 22, 'Forest Crossing'], [spineC - 1, 33, 'Beach Crossing']]) {
  ...
  map.hubs = [
    { name: 'Home', c: map.door.c, r: map.door.r },
    { name: 'Plaza', c: plazaCx, r: plazaCy },
    { name: 'Forest', c: map.raniDoor.c, r: map.raniDoor.r },
    { name: 'Beach', c: dock.c, r: dock.r0 },
  ];
