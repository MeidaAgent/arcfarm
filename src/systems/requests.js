...
export function requestText(r, canFill) {
  const head = `${r.npcName} left a note:`;
  const body = `"I need ${r.count} ${r.itemName}. Please bring them here if you can."`;
  const pay = `Reward: G${r.gold.toLocaleString('en-US')}.`;
  return canFill
    ? `${head}|${body}|${pay} Press E to deliver.`
    : `${head}|${body}|${pay} Bring them to the Village Hall once collected.`;
}
